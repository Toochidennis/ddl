...
public class ClassResultAdapter extends RecyclerView.Adapter<ClassResultAdapter.ClassResultViewHolder> {
    ...
    public ClassResultAdapter(Context context, List<ClassNameTable> classResultList,OnClassResultClickListener onClassResultClickListener) {
    ...
    public void onBindViewHolder(@NonNull ClassResultViewHolder classResultViewHolder, int i) {
        classResultViewHolder.name.setText(classResultList.get(i).getClassName().toUpperCase());
        classResultViewHolder.classCounter.setText(String.valueOf(i+1));

        GradientDrawable gd = (GradientDrawable) classResultViewHolder.counterBackground.getBackground().mutate();
    ...
    class ClassResultViewHolder extends RecyclerView.ViewHolder implements View.OnClickListener {
        private ImageView icon;
        private TextView name,classCounter;
        OnClassResultClickListener onClassResultClickListener;
        private LinearLayout counterBackground;
        public ClassResultViewHolder(@NonNull View itemView,OnClassResultClickListener onClassResultClickListener) {
            super(itemView);
